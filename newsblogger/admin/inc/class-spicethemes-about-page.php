...
if (!class_exists('NewsBlogger_About_Page')) {
	class NewsBlogger_About_Page {
		...
		function __construct() {
			...
			add_action('wp_ajax_newsblogger_update_rec_acts', array($this, 'update_recommended_actions_watch'));
			add_action('load-themes.php', array($this, 'activation_admin_notice'));
			/* enqueue script and style for welcome screen */
			add_action( 'admin_enqueue_scripts', array( $this, 'style_and_scripts' ) );
			
			/* load welcome screen */
			add_action( 'newsblogger_info_screen', array( $this, 'getting_started' ), 10 );
			add_action( 'newsblogger_info_screen', array( $this, 'recommended_actions' ), 20 );
			add_action( 'newsblogger_info_screen', array( $this, 'video_tutorials' ), 30 );
			add_action( 'newsblogger_info_screen', array( $this, 'unlock_premium_feature' ), 40 );
		}
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			if ( 'toplevel_page_newsblogger-welcome' == $hook_suffix || 'admin_page_spice-settings-importer' == $hook_suffix ) {
			
				wp_enqueue_style( 'newsblogger-info-screen-css', NEWSBLOGGER_TEMPLATE_DIR_URI . '/admin/assets/css/welcome.css' );
				wp_enqueue_style('newsblogger-theme-info-style', NEWSBLOGGER_TEMPLATE_DIR_URI . '/admin/assets/css/welcome-page-styles.css');
				wp_enqueue_style('newsblogger-welcome_customizer', NEWSBLOGGER_TEMPLATE_DIR_URI . '/admin/assets/css/welcome_customizer.css');
				wp_enqueue_script('plugin-install');
				wp_enqueue_script('updates');
				
			}
			wp_enqueue_script('newsblogger-blocks-install', NEWSBLOGGER_TEMPLATE_DIR_URI . '/admin/assets/js/plugin-install.js', array('jquery'));
			wp_enqueue_script('newsblogger-ajax', NEWSBLOGGER_TEMPLATE_DIR_URI . '/admin/assets/js/ajax.js', array('jquery'));
			wp_localize_script('newsblogger-blocks-install', 'newsblogger_companion_install',
			array(
				'installing' => esc_html__('Installing', 'newsblogger' ),
				'activating' => esc_html__('Activating', 'newsblogger' ),
				'error'      => esc_html__('Error', 'newsblogger' ),
		...
		public function welcome_admin_notice() {
			...
			<div class="updated notice is-dismissible">
				...
				printf( esc_html__('%1$s theme is installed. To take full advantage of the features this theme has to offer visit our %2$swelcome page%3$s', 'newsblogger'), esc_html($theme_name), '<a href="' . esc_url( admin_url( 'admin.php?page=newsblogger-welcome' ) ) . '" class="newsblogger-wl-page-link">', '</a>' ); ?></p>
				<p><a href="<?php echo esc_url(admin_url('admin.php?page=newsblogger-welcome')); ?>" class="button newsblogger-get-start-btn" style="text-decoration: none;"><?php 
				/* translators: %s: theme name */
				printf( esc_html__( 'Get started with %s theme', 'newsblogger'  ), esc_html($theme_name)); ?></a></p>
			</div>
			<?php
		}
	  		...
	  		<div class="spice-container-fulid">
				<div class="spice-row">
					<div class="spice-col-1">
						<div class="wrap theme-info-wrap newsblogger-wrap">
							...
							<div class="theme-welcome-container" style="min-height:300px;">
								<div class="theme-welcome-inner">
									...
									?>
									<ul class="newsblogger-nav-tabs" role="tablist">
										<?php echo wp_kses_post($tabs_head); ?>
									</ul>
									 
									<div class="newsblogger-tab-content">
										<?php do_action( 'newsblogger_info_screen' ); ?>
									</div>
		...
		public function get_recommended_plugins() {
			...
			$plugins = apply_filters('newsblogger_recommended_plugins', array());
			return $plugins;
		}

		public function get_useful_plugins() {
			$plugins = apply_filters('newsblogger_useful_plugins', array());
			return $plugins;
		}
		...
		public function get_plugin_buttion($slug, $name) {
			...
			elseif ($is_installed && !$is_activeted) {
    			...
    			{
    				$activate_string=esc_html__('Activate', 'newsblogger' );
    			}
    			else
    			{
    				$activate_string=esc_html__('Activate Import Plugin', 'newsblogger' );
    			}
				...
				$button_html = sprintf('<a class="spicethemes-plugin-activate activate-now button-primary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
					...
					/* translators: %s: theme name */
					sprintf(esc_html__('Activate %s now', 'newsblogger' ), esc_html($name)),
					esc_html($name),
					$activate_string
				);
			} 
			elseif ($is_activeted) {
    			...
    			if ( "themes.php" != $pagenow && is_admin() )
    			{
				$button_html = sprintf('<div class="action-link button disabled"><span class="dashicons dashicons-yes"></span> %s</div>', esc_html__('Active', 'newsblogger' ));
				$is_done     = true;
				}
			}
		...
		public function get_plugin_icon( $icons ) {
			...
			if ( ! empty( $icons['svg'] ) ) {
				...
				$plugin_icon_url = NEWSBLOGGER_TEMPLATE_DIR_URI . '/admin/assets/images/placeholder_plugin.png';
			}
...
function newsblogger_useful_plugins_array($plugins){
	$plugins[] = array(
		'slug'     => 'elementor',
	);
	return $plugins;
}
add_filter('newsblogger_useful_plugins', 'newsblogger_useful_plugins_array');

function newsblogger_recommended_plugins_array($plugins){
		
		$plugins[] = array(
			'name'     	=> 'Contact Form 7',
			'slug'     	=> 'contact-form-7',
			'desc'     	=> esc_html__('To display the contact form, install & activate this plugin.', 'newsblogger' ),
		);
	 	$plugins[] = array(
			'name'     => 'Yoast SEO',
			'slug'     => 'wordpress-seo',
			'desc'     => esc_html__('To enable the display of breadcrumbs, please install and activate this plugin.', 'newsblogger'),
	    );
		$plugins[] = array(
	        'name' 		=> 'Unique Headers',
	        'slug' 		=> 'unique-headers',
	        'desc' 		=> esc_html__('To add a different banner image for a different page or post, install & activate this plugin.', 'newsblogger'),
        );
	    $plugins[] = array(
			'name'     => 'Spice Post Slider',
			'slug'     => 'spice-post-slider',
			'desc'     => esc_html__('To display the posts in a beautiful slider with multiple options, install & activate this plugin.', 'newsblogger'),
	    );
	    $plugins[] = array(
			'name'     => 'Spice Social Share',
			'slug'     => 'spice-social-share',
			'desc'     => esc_html__('To add the social share buttons to your posts, install & activate this plugin.', 'newsblogger'),
	    );
	    $plugins[] = array(
			'name'     => 'Seo Optimized Images',
            'slug'     => 'seo-optimized-images',
            'desc'     => esc_html__('We highly recommend installing and activating the Seo Optimized Images plugin, which will dynamically insert SEO-friendly alt attributes and title attributes to enhance your image optimization.', 'newsblogger'),
		);
		$plugins[] = array(
			'name'     => 'WooCommerce',
			'slug'     => 'woocommerce',
...
add_filter('newsblogger_recommended_plugins', 'newsblogger_recommended_plugins_array');

function newsblogger_about_page() {
	return NewsBlogger_About_Page::get_instance();
}
global $newsblogger_about_page;
